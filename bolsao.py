...
def calcula_valor_minimo(unidade: str, serie_modalidade: str) -> float:
    """
    Retorna o menor valor de PARCELA (13x) permitido para a combinação de
    unidade + série/modalidade, considerando o desconto máximo por unidade.
    """
    try:
        desconto_maximo = DESCONTOS_MAXIMOS_POR_UNIDADE.get(unidade, 0.0)
        precos = precos_2026(serie_modalidade)
        valor_anuidade_integral = float(precos.get("anuidade", 0.0))

        if valor_anuidade_integral > 0 and 0 < desconto_maximo < 1:
            valor_minimo_anual = valor_anuidade_integral * (1 - desconto_maximo)
            # 13x = 1ª + 12 iguais -> o “mínimo negociável” que exibimos é a parcela (as 12 iguais)
            return valor_minimo_anual / 12.0
        return 0.0
    except Exception:
        return 0.0
